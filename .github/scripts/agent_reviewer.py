...
Do not output chain-of-thought, hidden reasoning, a thinking process, or a preamble. Start immediately with `## AI Verdict` and keep the entire response under 1200 words.
...
def gemini_review(prompt: str) -> tuple[str, str]:
    ...
    payload = {"contents": [{"role": "user", "parts": [{"text": prompt}]}], "generationConfig": {"temperature": 0.1, "maxOutputTokens": 3000}}
...
def validate_review_text(text: str) -> str:
    ...
    if not all(h in text for h in required):
        raise RuntimeError("Reviewer output is malformed or truncated")
    ...
    if len(verdicts) != 1:
        raise RuntimeError("Reviewer output must contain exactly one verdict in AI Verdict section")
    return text.strip()
...
def openrouter_review(prompt: str) -> tuple[str, str]:
    ...
    obj = post_json(
        ...
        {"model": OPENROUTER_MODEL, "temperature": 0.1, "max_tokens": 3000, "reasoning": {"effort": "none", "exclude": True}, "messages": [{"role": "user", "content": prompt}]},
    )
...
def get_ai_review(prompt: str) -> tuple[str, str, list[str]]:
    ...
    for fn in (gemini_review, openrouter_review):
        try:
            ...
            return provider, validate_review_text(text), errors
...
def main() -> int:
    ...
    ai_approved = "[APPROVED]" in verdict_section
