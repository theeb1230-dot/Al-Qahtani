...
void main() {
  ...
  test('invalid resume range deletes stale partial', () async {
    final root = await Directory.systemTemp.createTemp('al-qahtani-invalid-resume-test');
    addTearDown(() async { if (await root.exists()) await root.delete(recursive: true); });
    final partial = File('${root.path}${Platform.pathSeparator}.qahtani-series_ref_episode_5.part');
    await partial.writeAsBytes(<int>[1,2,3,4,5]);
    final service = DownloadService(client: _InvalidRangeClient(), directoryProvider: () async => root);
    addTearDown(service.close);
    await expectLater(
      service.download(Uri.parse('https://al-qahtani-api.onrender.com/api/cinema/media?id=fresh-opaque&download=1'), fallbackName: 'Episode 5', resumeKey: 'series/ref:episode:5'),
...
class _LaterAttemptClient extends http.BaseClient {
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    return http.StreamedResponse(Stream.value(<int>[6,7,8,9,10]), 206, request: request, contentLength: 5, headers: const {'content-range':'bytes 5-9/10','content-length':'5','content-disposition':'attachment; filename="episode-5.mp4"','accept-ranges':'bytes'});
  }
}
...
class _InvalidRangeClient extends http.BaseClient {
  @override
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    return http.StreamedResponse(Stream.value(<int>[6,7,8,9,10]), 206, request: request, contentLength: 5, headers: const {'content-range':'bytes 6-10/11','content-length':'5','content-disposition':'attachment; filename="episode-5.mp4"','accept-ranges':'bytes'});
  }
}
