...
export class ProviderHealthRegistry {
  ...
  recordSuccess(name, { latencyMs, capabilities = {}, now = Date.now() } = {}) {
    const entry = this.#entry(name);
    entry.successes += 1;
    entry.consecutiveFailures = 0;
    entry.lastSuccessAt = now;
    entry.openUntil = 0;
    if (Number.isFinite(Number(latencyMs))) entry.lastLatencyMs = Math.max(0, Number(latencyMs));
  ...
  score(name, now = Date.now()) {
    ...
    let score = 100;
    score += Math.min(30, entry.successes * 2);
    score -= Math.min(60, entry.failures * 4);
    score -= entry.consecutiveFailures * 15;
    if (Number.isFinite(entry.lastLatencyMs)) score -= Math.min(40, entry.lastLatencyMs / 250);
    if (entry.capabilities.range206) score += 8;
    if (entry.capabilities.safariPlayable) score += 12;
...
export function normalizeCatalogItem(item = {}) {
  return {
    id: asText(item.id || item.ref || item.href || item.link),
    title: asText(item.title || item.name) || "بدون عنوان",
    poster: asText(item.poster || item.img || item.image),
    type: item.is_series === false || item.type === "movie" ? "movie" : "series",
    year: asNumber(item.year),
    ref: asText(item.ref || item.href || item.link),
  };
}

export function normalizeEpisode(episode = {}, index = 0) {
  const episodeNumber = asNumber(episode.episode_number ?? episode.num ?? episode.number, index + 1);
  return {
    episode_id: asText(episode.episode_id || episode.id || episode.link || episode.href),
    episode_number: Math.max(1, Math.trunc(episodeNumber || index + 1)),
    title: asText(episode.title || episode.name) || `الحلقة ${Math.max(1, Math.trunc(episodeNumber || index + 1))}`,
    ref: asText(episode.ref || episode.link || episode.href),
    watch_available: episode.watch_available !== false && Boolean(episode.ref || episode.link || episode.href),
  };
}
...
export function buildRuntimeEnvelope({ kind, data, source, health, cached = false, generatedAt = Date.now() }) {
  return {
    status: "success",
    version: PRODUCT_VERSION,
    kind: asText(kind) || "unknown",
    source: asText(source) || "basri-original",
