...
export async function getCinemaCategory(type,name,url="",page=1){const p=Math.max(1,Number(page)||1);return fetchJsonWithHealth(ENDPOINTS.backend+"/api/cinema/category?type="+encodeURIComponent(type)+"&name="+encodeURIComponent(String(name||"").trim())+"&url="+encodeURIComponent(url)+"&p="+encodeURIComponent(String(p)),{kind:ProviderKind.CINEMA,timeoutMs:BACKEND_TIMEOUT_MS,validate:isCinemaPayload})}
