...
async function matchesSmoke() {
  const session = await json(MATCHES + "session", { headers: { Accept: "application/json", "X-BSR-Page": "/Al-Qahtani/" } });
  ...
  const list = await json(MATCHES, { headers: { Accept: "application/json", "X-BSR-Page": "/Al-Qahtani/", "X-BSR-Token": token } });
  ...
  if (playable?.link) {
    const servers = await json(playable.link, { headers: { Accept: "application/json", "X-BSR-Page": "/Al-Qahtani/", "X-BSR-Token": token } });
...
async function cinemaSmoke() {
  const session = await json(CINEMA + "session", { headers: { Accept: "application/json" } });
  ...
  const genre = await json(CINEMA + "?action=genre&genre=" + genreUrl + "&p=1&token=" + token, { headers: { Accept: "application/json" } });
  ...
  const search = await json(CINEMA + "?action=search&q=" + encodeURIComponent("الذئب الوحيد") + "&token=" + token, { headers: { Accept: "application/json" } });
  ...
  if (sample?.href) {
    const details = await json(CINEMA + "?action=series&series=" + encodeURIComponent(sample.href) + "&token=" + token, { headers: { Accept: "application/json" } });
...
async function newsSmoke() {
  const result = await json(NEWS + "?_=" + Date.now(), { headers: { Accept: "application/json" } });
