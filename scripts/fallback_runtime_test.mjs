...
const runtime = new FallbackPlaybackRuntime({ ttlMs: 5_000, maxAttempts: 3, now: () => now });
...
assert.equal(first.data.max_attempts, 3);
...
assert.equal(internal.attempt, 1);
...
const probe = await runtime.probe(first.data.ref, {
  fetchImpl: async (target, init) => {
    ...
    return { status: 206, headers: { get(name) { const key = String(name).toLowerCase(); if (key === "content-type") return "video/mp4"; if (key === "content-range") return "bytes 0-1023/4096"; if (key === "accept-ranges") return "bytes"; return null; } }, body: { cancel: async () => {} } };
  },
});
assert.match(probedTarget, /^https:\/\//);
...
assert.equal(probe.data.attempt, 1);
assert.equal(JSON.stringify(probe).includes("https://"), false);
assert.equal(JSON.stringify(probe).includes(internal.providerId), false);
...
const direct = await runtime.openDirectMedia(first.data.ref, {
  ...
  fetchImpl: async (target, init) => {
    ...
    if (directCalls === 1) {
      assert.equal(init.headers.Range, "bytes=0-1023");
      return new Response(new Uint8Array([0, 0, 0, 16, 102, 116, 121, 112, 105, 115, 111, 109]), { status: 206, headers: { "content-type": "video/mp4", "content-range": "bytes 0-1023/4096", "accept-ranges": "bytes" } });
    }
    assert.equal(init.headers.Range, "bytes=1024-2047");
    return new Response(new Uint8Array([1, 2, 3, 4]), { status: 206, headers: { "content-type": "video/mp4", "content-length": "4", "content-range": "bytes 1024-1027/4096", "accept-ranges": "bytes" } });
  },
});
...
assert.throws(() => runtime.recordSuccess(first.data.ref, { playbackSignal: true }), /PLAYING_EVIDENCE_REQUIRED/);
assert.deepEqual(runtime.recordSuccess(first.data.ref, { playbackSignal: true, playing: true, latencyMs: 120, container: "embed" }), { status: "success" });

assert.throws(() => runtime.recordFailure(first.data.ref), /PLAYER_FAILURE_EVIDENCE_REQUIRED/);
const second = runtime.recordFailure(first.data.ref, { playerFailure: true });
...
const third = runtime.recordFailure(second.data.ref, { playerFailure: true });
assert.equal(third.data.attempt, 3);
assert.throws(() => runtime.recordFailure(third.data.ref, { playerFailure: true }), /FALLBACK_ATTEMPT_LIMIT_REACHED/);
assert.throws(() => runtime.next(third.data.ref), /FALLBACK_ATTEMPT_LIMIT_REACHED/);
...
now += 5_001;
assert.throws(() => runtime.inspect(third.data.ref), /FALLBACK_REFERENCE_EXPIRED/);
