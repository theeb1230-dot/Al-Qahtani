...
class _NewsPageState extends State<NewsPage> {
  ...
  Widget build(BuildContext context) => FutureBuilder<List<NewsItem>>(
        ...
        builder: (context, snapshot) {
          ...
          if (items.isEmpty) return _NewsMessage(message: 'لا توجد أخبار متاحة الآن', onRetry: retry);
                ...
                return Card(
                  child: ListTile(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        if (item.date.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 6), child: Text(item.date)),
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: FutureBuilder<NewsArticle>(
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) return const _NewsMessage(message: 'تعذر تحميل تفاصيل الخبر');
            final article = snapshot.data!;
            return ListView(
              padding: EdgeInsets.all(isTvTarget ? 28 : 18),
              children: [
                Text(
                  article.title.isEmpty ? item.title : article.title,
                  style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold),
                ),
                if ((article.date.isEmpty ? item.date : article.date).isNotEmpty) ...[
                  const SizedBox(height: 8),
                  Text(article.date.isEmpty ? item.date : article.date, style: Theme.of(context).textTheme.bodySmall),
                ],
